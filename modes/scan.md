...
**Every company MUST have `careers_url` in portals.yml.** If missing, search for it once, save it, and use in future scans.
...
- **Ashby**: `"Regulatory Affairs Specialist (Remote) @ BioTech Inc"` → title: `Regulatory Affairs Specialist`, company: `BioTech Inc`
- **Greenhouse**: `"Medical Writer at Moderna"` → title: `Medical Writer`, company: `Moderna`
- **Lever**: `"Document Control Specialist - Quality @ Dexcom"` → title: `Document Control Specialist - Quality`, company: `Dexcom`
...
https://...	2026-02-10	Ashby — Reg Affairs	Regulatory Specialist	Acme	added
https://...	2026-02-10	Greenhouse — MedWriter	Junior Content Writer	BigCo	skipped_title
https://...	2026-02-10	Ashby — Quality	QA Doc Specialist	OldCo	skipped_dup
```

## Output Summary (keep it SHORT)

```
Scan — {YYYY-MM-DD} | {N} new added | {N} skipped | {N} duplicate

  + {company} | {title}
  ...

→ /career-ops pipeline to evaluate.
```

No per-query breakdowns unless the user asks.

## careers_url Management

Every company in `tracked_companies` must have `careers_url` — the direct URL to their job listings page. This avoids searching for it each time.
...
- Add companies to `tracked_companies` when they become of interest
